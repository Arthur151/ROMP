...
class MotChallenge2DBox(_BaseDataset):
    ...
    def get_default_dataset_config():
        ...
        default_config = {
            'GT_FOLDER': 'data/t3dp_tracking_gts',  # Location of GT data
            'TRACKERS_FOLDER': os.path.join(code_path, 'data/trackers/mot_challenge/'),  # Trackers location
            'OUTPUT_FOLDER': None,  # Where to save eval results (if None, same as TRACKERS_FOLDER)
            'TRACKERS_TO_EVAL': None,  # Filenames of trackers to eval (if None, all in folder)
            'CLASSES_TO_EVAL': ['pedestrian'],  # Valid: ['pedestrian']
            'BENCHMARK': 'PoseTrack',  # Valid: 'MOT17', 'MOT16', 'MOT20', 'MOT15'
    ...
    def __init__(self, config=None):
        ...
        if self.config['TRACKER_SUB_FOLDER'] is None:
            self.tracker_sub_fol = 'MultiPersonTestSet' if self.benchmark=='MuPoTS' else 'posetrack2018'
        else:
        ...
        for tracker in self.tracker_list:
            if self.data_is_zipped:
                ...
                for seq in self.seq_list:
                    ...
                    if not os.path.isfile(curr_file):
                        print('Tracker file not found: ' + curr_file)
                        raise TrackEvalException('Tracker file not found: '+curr_file)
        ...
        if self.config["SEQ_INFO"]:
            ...
            with open(seqmap_file) as fp:
                ...
                for i, row in enumerate(reader):
                    if i == 0 or row[0] == '':
                        continue
                    seq, seq_length = row[0].split(' ')
                    seq_lengths[seq] = int(seq_length)
                    seq_list.append(seq)
                    #ini_file = os.path.join(self.gt_fol, seq, 'seqinfo.ini')
                    #if not os.path.isfile(ini_file):
                    #    raise TrackEvalException('ini file does not exist: ' + seq + '/' + os.path.basename(ini_file))
