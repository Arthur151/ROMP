...
from lib.parallel.balanced_dataparallel import DataParallel
...
from lib.datasets.mixed_dataset import MixedDataset, SingleDataset
...
class Base(object):
    def __init__(self):
        ...
        if self.video:
            self.net_forward = self.network_forward_video
    ...
    def _build_model_(self):
        ...
        model = build_model(self.backbone, self.model_version)
        if self.fine_tune or self.eval:
            drop_prefix = ''
            if self.model_version==6:
                model = load_model(self.model_path, model, prefix = '', drop_prefix=drop_prefix, fix_loaded=True)#'module.'
            else:
        ...
        if self.distillation_learning:
            logging.info('start building teacher model.')
            teacher_model = build_teacher_model()
            drop_prefix = 'backbone.'
            logging.info('loading pre-trained teacher model.')
            teacher_model = load_model(self.teacher_model_path, teacher_model, prefix = 'module.', drop_prefix=drop_prefix, fix_loaded=False)
            if self.master_batch_size!=-1:
                self.teacher_model = DataParallel(teacher_model.cuda(), device_ids=self.gpus, chunk_sizes=self.chunk_sizes)
            else:
                self.teacher_model = nn.DataParallel(teacher_model.cuda())
            logging.info('Fninish building teacher model.')
    ...
    def _init_params(self):
        ...
        self.val_cfg = {'mode':'parsing', 'calc_loss': False, 'with_nms':True}
        ...
        if self.video:
            self.chunk_sizes = [i*self.temp_clip_length for i in self.chunk_sizes]
        print('training chunk_sizes:{}'.format(self.chunk_sizes))

        self.lr_hip_idx = np.array([constants.SMPL_ALL_44['L_Hip'], constants.SMPL_ALL_44['R_Hip']])
        ...
        self.All44_to_LSP14_mapper = constants.joint_mapping(constants.SMPL_ALL_44, constants.LSP_14)

        #self.J_regressor_h36m = torch.from_numpy(np.load(os.path.join(config.model_dir, 'spin_data', 'J_regressor_h36m.npy'))).float()
    ...
    def network_forward_video(self, video_model, image_model, meta_data, cfg_dict):
        ds_org, imgpath_org = get_remove_keys(meta_data,keys=['data_set','imgpath'])
        meta_data['batch_ids'] = torch.arange(len(meta_data['image']))
        image_outputs = image_model(meta_data, {'mode':'forward'})
        outputs = video_model(image_outputs, meta_data, **cfg_dict)
        meta_data.update({'imgpath':imgpath_org, 'data_set':ds_org})
        outputs['meta_data']['data_set'], outputs['meta_data']['imgpath'] = reorganize_items([ds_org, imgpath_org], outputs['reorganize_idx'].cpu().numpy())
        return outputs
    ...
    def _create_data_loader(self,train_flag=True):
        ...
        datasets = MixedDataset(self.datasets.split(','), self.sample_prob_dict, train_flag=train_flag)
        ...
        if self.distributed_training:
            ...
            if self.relative_depth_scale_aug:
                batch_sampler = RepeatedBatchSampler(datasets, batch_size)
                return DataLoader(dataset=datasets, batch_sampler=batch_sampler, pin_memory=True,num_workers=self.nw)
            else:
                return DataLoader(dataset = datasets,\
                    batch_size = batch_size, shuffle = True, \
                    drop_last = True if train_flag else False, pin_memory = True,num_workers = self.nw)

    def _create_single_data_loader(self, shuffle=False, drop_last=True, **kwargs):
    ...
    def set_up_val_loader(self):
        ...
        if 'jta' in eval_datasets:
            self.dataset_val_list['jta'] = self._create_single_data_loader(dataset='jta', split='val', train_flag = False)
            self.dataset_test_list['jta'] = self._create_single_data_loader(dataset='jta', split='test', train_flag = False)
            self.val_best_PAMPJPE['jta'] = 80
        if 'oh' in eval_datasets:
            self.dataset_val_list['oh'] = self._create_single_data_loader(dataset='oh', train_flag=False, split='val',joint_format='smpl24')
        ...
        if 'cmup' in eval_datasets:
            self.dataset_val_list['cmup'] = self._create_single_data_loader(dataset='cmup', train_flag=False, split='test', drop_last=True)
            #self._create_single_data_loader(dataset='cmup', train_flag=False, split='val')
        ...
        if 'pw3d_pc' in eval_datasets:
            self.dataset_val_list['pw3d_pc'] = self._create_single_data_loader(dataset='pw3d', train_flag = False, split='all', mode='PC', load_vertices=True)
        if 'pw3d_nc' in eval_datasets:
            self.dataset_val_list['pw3d_nc'] = self._create_single_data_loader(dataset='pw3d', train_flag = False, split='all', mode='NC',regress_smpl=False)
        if 'pw3d_oc' in eval_datasets:
            self.dataset_val_list['pw3d_oc'] = self._create_single_data_loader(dataset='pw3d', train_flag = False, split='all', mode='OC',regress_smpl=False)
        if 'crowdpose' in eval_datasets:
            self.dataset_val_list['crowdpose'] = self._create_single_data_loader(dataset='crowdpose', train_flag = False, split='val')
            self.dataset_test_list['crowdpose'] = self._create_single_data_loader(dataset='crowdpose', train_flag = False, split='test')
        if 'pw3d' in eval_datasets:
            self.dataset_val_list['pw3d'] = self._create_single_data_loader(dataset='pw3d', train_flag=False, mode='vibe', split='test',regress_smpl=False)
            ...
            self.val_best_PAMPJPE['pw3d_vibe'] = 50
        ...
        if 'mini' in eval_datasets:
            self.dataset_val_list['mini'] = self._create_single_data_loader(dataset='mini', train_flag=False, split='val')
            self.dataset_val_list['mini'] = self._create_single_data_loader(dataset='mini', train_flag=False, split='test')
            self.val_best_PAMPJPE['mini'] = 30.
