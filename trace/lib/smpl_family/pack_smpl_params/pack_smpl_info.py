...
torch.save(tensor_model_info, 'SMPL_{}.pth'.format(gender.upper()))


kid_template_path = os.path.join(root_folder, 'smpl_kid_template.npy')
...
#np.savez('smpla_packed_info.npz', annots=np_model_info)
...
for k, v in np_model_info.items():
    ...
    if k in ['kintree_table', 'extra_joints_index']:
        ...
        tensor_model_info[k] = torch.from_numpy(v).float()
torch.save(tensor_model_info, 'SMPLA_{}.pth'.format(gender.upper()))