...
class FlowExtract(nn.Module):
    ...
    def forward(self, images, seq_inds):
        ...
        target_img_inds = torch.arange(len(input_images))#.cuda()
        ...
        #print(num, input_images[source_img_inds[:num]].contiguous().shape, input_images[target_img_inds[:num]].contiguous().shape)
