    ...
    def __call__(self, verts, faces, colors=torch.Tensor(mesh_color_table['neutral']), merge_meshes=True, cam_params=None):
        ...
        if cam_params is not None:
            if self.perps:
                ...
                new_cam = FoVOrthographicCameras(R=R, T=T, zfar=100.0, max_y=2.0, min_y=-2.0, max_x=2.0, min_x=-2.0, device=self.device)
...
# class Points3DRenderer(nn.Module):
#     def __init__(self, resolution=(512,512), R=None, T=None, fov=args().FOV, point_radius=0.02, use_gpu=args().gpu!='-1'):
#         super(Points3DRenderer, self).__init__()
#         if use_gpu:
#             self.device = torch.device('cuda:{}'.format(args().gpu.split(',')[0]))
#         else:
#             self.device = torch.device('cpu')
...
#     def __call__(self, verts, colors, merge_meshes=True, cam_params=None):
#         """
#         verts: a tensor of shape (num_points, 3) containing the (x, y, z) coordinates of each point.
#         colors: a tensor of shape (num_points, C) containing the features for the points in the cloud.
#         points :List where each element is a tensor of shape (num_points, 3) containing the (x, y, z) coordinates of each point.
#                 Padded float tensor with shape (num_clouds, num_points, 3).
#         features: List where each element is a tensor of shape (num_points, C) containing the features for the points in the cloud.
...
#             R, T, fov = cam_params
#             new_cam = FoVPerspectiveCameras(R=R, T=T, fov=fov, device=self.device)
#             images = self.renderer(point_cloud,cameras=new_cam)
#         else:
