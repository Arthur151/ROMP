...
def normalize_kps(kps,image_shape,resize=512,set_minus=True):
    kps[:,0] = kps[:,0] * 1.0 * resize / image_shape[0]
    kps[:,1] = kps[:,1] * 1.0 * resize / image_shape[1]
...
class Timer(object):
    ...
    def __call__(self):
        print('time spent:',time.time()-self.start_time)
        self.start_time = time.time()
...
def write_words2img(img,height_use,words,line_length=20,line_height=36,width_min=1420,color=(0, 0 ,0),duan_space=True):
    font = ImageFont.truetype("/home/sunyu15/shoes_attributes/data/song.ttf", 28)
