    ...
    def forward(self, betas=None, thetas=None, root_align=True):
        if betas.shape[-1] > 10:
            ...
            if baby_mask.sum()>0:
                ...
                verts, joints = torch.zeros(person_num, 6890, 3, device=thetas.device).float(), torch.zeros(person_num, args().joint_num+17, 3, device=thetas.device).float()
...
def prepare_smpl_model(dtype):
    model_path = '/home/yusun/Infinity/project_data/romp_data/model_data/parameters/smpl_packed_info.pth'
