...
def resize2tracking_format(pred_batch_ids, pred_czyxs, clip_length, seq_masks, image_repeat=args().image_repeat_time):
    seq_tracking_results = {}
    seq_id = 0
    for ind in range(torch.div(len(pred_batch_ids[seq_masks]), clip_length, rounding_mode='floor')):
        batch_inds = pred_batch_ids[seq_masks][ind*clip_length:(ind+1)*clip_length]
        clip_czyxs = pred_czyxs[seq_masks][ind*clip_length:(ind+1)*clip_length]
        seq_mask = seq_masks[seq_masks][ind*clip_length:(ind+1)*clip_length]
        seq_id = torch.div((max(batch_inds).item()+1), clip_length, rounding_mode='floor')
        if seq_id not in seq_tracking_results:
            seq_tracking_results[seq_id] = [{},{},{}]
        clip_id = len(seq_tracking_results[seq_id][0])
        seq_tracking_results[seq_id][0][clip_id] = clip_czyxs
        seq_tracking_results[seq_id][1][clip_id] = batch_inds
        seq_tracking_results[seq_id][2][clip_id] = True
    
    img_init_seq_id = int(seq_id)
    for ind in range(len(pred_batch_ids[~seq_masks]) // image_repeat):
        batch_inds = pred_batch_ids[~seq_masks][ind*image_repeat:(ind+1)*image_repeat]
        clip_czyxs = pred_czyxs[~seq_masks][ind*image_repeat:(ind+1)*image_repeat]
        seq_mask = seq_masks[~seq_masks][ind*image_repeat:(ind+1)*image_repeat]
        seq_id = img_init_seq_id + (max(batch_inds).item()+1) // image_repeat
        if seq_id not in seq_tracking_results:
            seq_tracking_results[seq_id] = [{},{},{}]
        clip_id = len(seq_tracking_results[seq_id][0])
        seq_tracking_results[seq_id][0][clip_id] = clip_czyxs
        seq_tracking_results[seq_id][1][clip_id] = batch_inds
        seq_tracking_results[seq_id][2][clip_id] = False

    return seq_tracking_results
...
def parse_tracking_ids(seq_tracking_ids,pred_batch_ids, clip_length):
    ...
    for frame_id in frame_ids:
        ...
        # matched_inds, org_batch_inds = get_org_batch_inds(trans_tracked, batch_ids, org_trans)
        # matched_czyxs = czyxs[matched_inds]

        # for track_id in seq_tracking_results:
        #     if track_id in detected_track_ids:
        #         seq_tracking_results[track_id][frame_id] = matched_czyxs[detected_track_ids==track_id]
        #         seq_tracking_batch_inds[track_id][frame_id] = org_batch_inds[detected_track_ids==track_id]
        #         valid_tracked_num[track_id] += 1
        #     else:
        #         # in-filling the missed ones via interpolating feature instead of center 
        #         # in-filling the missed detection to make the 3D trajectory of all detected subjects complete
        #         if frame_id == 0:
        #             continue
        #             #seq_tracking_results[track_id][frame_id] = matched_czyxs[torch.where(trans_track_ids[:,3] == track_id)[0][0]]
        #             #seq_tracking_batch_inds[track_id][frame_id] = 0
        #         else:
        #             seq_tracking_results[track_id][frame_id] = seq_tracking_results[track_id][frame_id-1]
        #             if seq_tracking_batch_inds[track_id][frame_id-1] != -1:
        #                 seq_tracking_batch_inds[track_id][frame_id] = seq_tracking_batch_inds[track_id][frame_id-1]+1
    
    #print(seq_tracking_results, seq_tracking_batch_inds,valid_tracked_num)
...
def perform_tracking(motion_offsets, pred_batch_ids, pred_cams, pred_czyxs, top_score, \
            batch_num=1, clip_length=8, seq_cfgs=None, tracker=None, debug=args().debug_tracking):
    ...
    for seq_id in torch.unique(pred_seq_ids):
        ...
        for clip_id in seq_clip_ids:
            ...
            if not args().tracking_with_kalman_filter:
                trans_track_ids = tracker.update(trans3D_current_frame.detach().cpu().numpy(), \
                    ...
                    debug=debug, never_forget=False, tracking_target_max_num=seq_cfgs['tracking_target_max_num'],\
                    using_motion_offsets=args().using_motion_offsets_tracking)
            else:
                trans_track_ids = tracker.update_BT(trans3D_current_frame.detach().cpu().numpy(), \
                        scores.detach().cpu().numpy(), trans3D_last_frame.detach().cpu().numpy(), czyxs, \
                        debug=debug)
...
"""
def prepare_complete_trajectory_features(self, seq_tracking_results, mesh_feature_maps):
    seq_traj_features = {}
    seq_traj_czyxs = {}
    seq_traj_batch_inds = {}
    seq_traj_valid_masks = {}
        ...
        for track_id in center_traj3Ds:
            ...
            for sbid in range(len(subj_batch_inds)):
                ...
                if sbid == 0 and subj_batch_ind == -1:
                    ...
                    fore_subj_batch_inds[sbid] = subj_batch_inds[valid_ind]
                    after_subj_batch_inds[sbid] = subj_batch_inds[valid_ind]
                    subj_batch_inds[0] = subj_batch_inds[valid_ind]
                    valid_mask[0] = False
                if subj_batch_ind == -1:
                    valid_fore_ind = torch.where(subj_batch_inds[:sbid] != -1)[0].max()
                    if len(torch.where(subj_batch_inds[(sbid+1):] != -1)[0])>0:
                    ...
                    after_subj_batch_inds[sbid] = subj_batch_inds[valid_after_ind]
                    #print(fore_subj_center3D_traj_weight[sbid].item(), fore_subj_batch_inds[sbid], after_subj_batch_inds[sbid], after_subj_center3D_traj_weight[sbid].item())
                    subj_batch_inds[sbid] = torch.floor(fore_subj_center3D_traj_weight[sbid].item() * fore_subj_batch_inds[sbid] + \
                                        after_subj_center3D_traj_weight[sbid].item() * after_subj_batch_inds[sbid])
                    valid_mask[sbid] = False
            fore_subj_features = self.image_feature_sampling(mesh_feature_maps, fore_subj_center3D_traj, fore_subj_batch_inds)
            after_subj_features = self.image_feature_sampling(mesh_feature_maps, after_subj_center3D_traj, after_subj_batch_inds)
            subj_features = fore_subj_features * fore_subj_center3D_traj_weight.unsqueeze(1) + \
    ...
    for seq_id, subj_features_list in seq_traj_features.items():
        ...
        traj_seq_masks = traj_seq_masks + seq_masks[seq_id]
        sample_seq_masks.append(torch.Tensor(seq_masks[seq_id]).sum()>0)
        #seq_ids.append(torch.ones(len(subj_features_list)).long() * seq_id)
    ...
    traj_seq_masks = torch.Tensor(traj_seq_masks).bool().reshape(-1)
    sample_seq_masks = torch.Tensor(sample_seq_masks).bool().reshape(-1)
    return traj_features, traj_czyxs, traj_batch_inds, traj_masks, traj_seq_masks, sample_seq_masks
"""